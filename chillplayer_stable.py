...
#				consolen_reset = call(['reset'])	
	...
	def video_puffer(self):
		...
		try:
			...
			print('Gesamtzeit nicht gefunden oder falsch gematcht oder sowas')			
			gesamtzeit = 10*60	

		player = str(parser.get('videooptions','player'))
		
		Pufferanzahl = 100	# Wie oft die untere Gleichung geprüft werden soll
			...
			for line in self.ladeprozess.stderr:
				...
				if (restzeit != None):
					...
					try: 
						...
						if player == "0":
							sys.stdout.write("\t\t\tVideo wird abgespielt in %s Sekunden\r" % (str(restzeit)))
						if (player == "1") and (restzeit < 0.5 * gesamtzeit) : 	# Restladezeit kleiner als die Hälfte der Gesamtabspielzeit
	...
	def starte_video(self):
		...
		player = str(parser.get('videooptions','player'))
		if player == "0":
			self.abspielprozess = call(['omxplayer','-o','local','-b', self.titel]) # wenn hier gepiped wird, dann kann man den Player nicht mehr kontrollieren
		if player == "1":
